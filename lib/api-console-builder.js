...
class ApiConsoleBuilder {
  ...
  outputTimeout() {
    ...
    console.log('Still working.', place);
  }
    ...
    .catch((cause) => {
      ...
      .then(() => {
        if (this.opts.verbose) {
          ...
          console.error(colors.red(cause.message));
          console.error(colors.red(cause.stack));
        }
  ...
  buildPolymer() {
    ...
    this.log('Storing build in the ' + this.tempOutput + ' folder');
  ...
  copyDependencies() {
    ...
    .then(() => {
      this.log('Dependency files copied.');
    });
  }
