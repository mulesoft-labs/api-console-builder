...
class TemplateManager {
  ...
  copyTemplate() {
    if (this.opts.embedded) {
      return this._copyImport();
    }
    return this._copyTemplateFile()
    .then(() => this._copyImport());
  }
  /**
  ...
  _copyTemplateFile() {
    ...
    this.logger.info(`Copying template file ${tpl} as index.html`);
   ...
   */
  _copyImport() {
    this.logger.info(`Copying import script for the template.`);
  ...
  processTemplate(vars) {
    ...
    this.logger.info('Processing API variables in the main file.');
