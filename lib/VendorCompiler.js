...
export class VendorCompiler {
   ...
   * @param {Winston} logger Logger object
   */
   ...
   * @return {Promise<void>} Promise resolved when the vendor file is created.
   */
  async compile() {
    const { workingDir, logger } = this;
    logger.debug('Preparing vendor.js file...');
    const resolveOpts = {
      paths: [path.join(workingDir, 'node_modules')],
    };
    const code = {};
    for (let i = 0, len = CryptoFiles.length; i < len; i++) {
      const file = CryptoFiles[i];
      const full = require.resolve(file, resolveOpts);
