...
class ApiConsoleBuilder {
  ...
  buildBundle(polymerProject, bundle) {
    ...
    this.logger.info(`(${buildName}) Initializing the build...`);
    ...
    if (buildName === 'es5-bundle') {
      this.logger.info(`(${buildName}) Adding custom elements adapter.`);
    ...
    buildStream.on('error', (e) => {
      this.logger.error(`(${buildName}) ${e.message}`);
      this.logger.error(e);
    });
    ...
    if (bundle.basePath) {
      this.logger.info(`(${buildName}) Setting up base path.`);
    ...
    if (bundle.addPushManifest) {
      this.logger.info(`(${buildName}) Adding push manifest.`);
