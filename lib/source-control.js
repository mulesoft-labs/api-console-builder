...
class SourceControl {
  ...
  installConsole(tag) {
    ...
    npm.load({
      ...
      const cmd = this._preparePackageName(tag);
      npm.commands.install([cmd], function (er, data) {
  ...
  _preparePackageName(version) {
    const pre = semver.prerelease(version);
    let result;
    if (pre) {
      result = `mulesoft/api-console#${version}`;
    } else {
      result = `api-console@${version}`;
    }
    return result;
  }
}
