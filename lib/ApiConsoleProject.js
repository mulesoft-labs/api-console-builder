...
export class ApiConsoleProject {
  /**
   * @param {(ProjectConfiguration|BuilderOptions)=} opts User configuration
   */
  constructor(opts={}) {
    let config = /** @type BuilderOptions */ (opts);
    if (!(config instanceof BuilderOptions)) {
      config = new BuilderOptions(opts);
    }
    this.opts = config;
    this.logger = this.__setupLogger(config);
   ...
   *
   * @param {String} workingDir The temporary working dir
   * @return {Promise<void>}
   */
   ...
   * @param {String} workingDir The temporary working dir
   * @return {Promise<void>} Resolved promise when all pre-build work has been
   * completed.
   */
  ...
  async postBuild(workingDir) {
    ...
    this.logger.info(`Removing destination location ${destination}...`);
    await fs.remove(destination);
    const src = path.join(workingDir, 'dist');
    this.logger.debug(`Copying bundled files from ${src} to ${destination}...`);
    await fs.copy(src, destination);
    this.logger.debug(`Bundle coppied.`);
