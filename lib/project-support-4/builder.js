...
class ApiConsoleBuilder {
  ...
  setOptymisationConditions() {
    ...
    const stripCommentsCondition = optEnabled &&
      !this.opts.noHtmlOptimization && htmlRe;
    const htmlMinifierCondition = optEnabled &&
      !this.opts.noHtmlOptimization && htmlRe;
    const jsCompileCondition = optEnabled &&
      !this.opts.noJsOptimization && jsRe;
    const cssSlamCssConfition = optEnabled &&
      !this.opts.noCssOptimization && cssRe;
    const cssSlamHtmlConfition = optEnabled &&
      !this.opts.noCssOptimization && htmlRe;
    this._optConditions = {
      htmlRe: htmlRe,
      jsRe: jsRe,
  ...
  /**
   * Builds the sources using Polymer builder.
   * @return {Promise}
   */
  buildPolymer() {
    ...
    return new Promise((resolve, reject) => {
      ...
      .on('error', function(err) {
        console.error('Dependency stream jejoin error: ' + err.message);
        reject(new Error(err.message));
      });
   ...
   * need to set up it manually in the source code because script will
   * not recognize paths in the right way.
   *
   * @return {Stream}
   */
  setElementPaths() {
    const bowerLocationAttribute = this.opts.findAttribute('bower-location');
    let bowerLocation;
   ...
   */
  clearBuildFiles() {
    const basePath = path.join(this.workingDir, this.workingBuildOutput);
    const folder = path.join(basePath, 'bower_components');
    const files = [
      path.join(basePath, 'api-console.html'),
      path.join(basePath, 'api-console-request.html'),
      path.join(basePath, 'api-console-styles.html')
    ];
   ...
   */
  copyDependencies() {
    return this._copyBowerFiles()
    .then(() => this._copyPrims());
  }
  /**
   * Copies list of files defined in `bowerDependencies` property to
   * destination components directory.
   * @return {Promise} Promise resolved when the files are copied.
   */
  _copyBowerFiles() {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        .pipe(gulp.dest((file) => {
          ...
          return path.dirname(path.join(this.workingBuildOutput, fpath));
        }));
        copyStream.on('end', () => {
  ...
  /**
   * Copies prism dependency to destination components directory.
   * @return {Promise} Promise resolved when the files are copied.
   */
  _copyPrims() {
    this.logger.info('Copying prism dependencies...');
    const files = [
      'bower_components/prism/*',
      'bower_components/prism/**'
    ];
    const destPath = path.join(
      this.workingBuildOutput, 'bower_components/prism');
    return new Promise((resolve, reject) => {
      const copyStream = gulp.src(files)
      .pipe(gulp.dest(destPath));
      copyStream.on('end', () => {
        this.logger.info('Prism files copied.');
        resolve();
      });
      copyStream.on('error', reject);
    });
