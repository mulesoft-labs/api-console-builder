...
class RamlSource {
  ...
  /**
   * Creates a path of the `api.json` file. It depends on current options. If the JSON is not
   * enabled or if inline JSON option is set it will not produce the `api.json` file so this
   * function return with `undefined`.
   *
   * @param {String} outputDir File location directory
   * @return {String|undefined} If current configuration allows creating `api.json` file then the
   * location of the file or undefined otherwise.
   */
  apiOutputPath(outputDir) {
    var apiJsonFile;
    if (this.opts.useJson && !this.opts.inlineJson) {
      apiJsonFile = path.join(outputDir, 'api.json');
    }
    return apiJsonFile;
  }
}
