...
class ApiConsoleBuilderOptions {
  constructor(opts) {
    ...
    /**
     * If true then the application will perform JavaScript compilation using Google Closure
     * Compiler.
     * Note that this will make build process much longer.
     *
     * Defaults to `false`.
     */
    this.compileJavaScript = opts.compileJavaScript;
    /**
     * Level of compilation passed to the Google Closure Compiler. Only valid if `compileJavaScript`
     * is set.
     ...
     */
    this.compileJavaScripLevel = opts.compileJavaScripLevel;
  }
  ...
  _setDefaults(opts) {
    ...
    if (!('compileJavaScript' in opts)) {
      opts.compileJavaScript = false;
    }
    if (!('compileJavaScripLevel' in opts)) {
