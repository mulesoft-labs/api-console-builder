...
class ApiConsoleBuilderOptions {
  constructor(opts) {
     ...
     * in the console.
     *
     * Use this option to optymise console's load time. It will not include `raml-json-enhance` and
    ...
    /**
     * If set it will not perform any code optimisation. It will disable:
     * - comments removal
     * - JS minification
     * - HTML minification
     * - CSS minification
  ...
  _setDefaults(opts) {
    ...
    if (!('jsCompilationLevel' in opts)) {
      opts.compileJavaScripLevel = 'WHITESPACE_ONLY';
    }
    if (!('noOptimisation' in opts)) {
      opts.noOptimisation = false;
    }
    if (!('noCssOptimisation' in opts)) {
      opts.noCssOptimisation = false;
    }
    if (!('noHtmlOptimisation' in opts)) {
      opts.noHtmlOptimisation = false;
    }
    if (!('noJsOptimisation' in opts)) {
      opts.noJsOptimisation = false;
    }
    return opts;
  }
}
