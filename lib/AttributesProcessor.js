...
export class AttributesProcessor {
   ...
   * @return {parse5.Document} The `parse5` document object.
   */
  createAst(content) {
    this.logger.debug('Main file read. Parsing content');
    return /** @type parse5.Document */ (parse5.parse(content));
  }
   ...
   *
   * @param {parse5.Element|parse5.Document} node Currently iterated node
   * @return {parse5.Element|undefined} A node of the `api-console` or undefined if there
   * was no API Console element in the document.
   */
  findConsole(node) {
    if (node.nodeName === 'api-console-app') {
      return /** @type parse5.Element */ (node);
    }
    if (node.childNodes && node.childNodes.length) {
      for (let i = 0, len = node.childNodes.length; i < len; i++) {
        const item = /** @type parse5.Element */ (node.childNodes[i]);
   ...
   *
   * @param {parse5.Element} node API Console node
