...
export class AttributesProcessor {
   ...
   */
  constructor(opts, logger, appMainFile) {
    /**
     * @type {BuilderOptions}
     */
    this.opts = opts;
  ...
  async setAttributes() {
    ...
    const content = await this.readMainFile();
    const doc = this.createAst(content);
    const node = this.findConsole(doc);
    if (!node) {
      this.logger.warn('The api-console element not found in the main file. Skipping attribute setting');
      return;
    }
   ...
   */
  async readMainFile() {
    return await fs.readFile(this.appMainFile, 'utf8');
  }
  /**
