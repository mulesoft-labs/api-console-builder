...
class ApiConsoleSourceControl {
   ...
   */
  copyLocalConsole() {
    this.log('Copying local files of the console.');
    ...
    .then((stats) => {
      ...
      this.log('Copying files from ', src);
      return fs.copy(src + '/', this.tmpDir);
    });
  }

  copyZipFile() {
    this.log('Copying local zip file of the console.');
  ...
  handleTemplates() {
    ...
    .then(() => {
      if (this.exampleFile) {
        this.log('Copy the example file to the build dir.');
  ...
  updateTemplateVars(raml) {
    ...
    .then(() => {
      if (this.opts.embedded) {
        this.log('Updating example file.');
