...
class ApiConsoleSourceControl {
  ...
  createConsoleDir() {
    ...
    .then(() => {
      this.log('===============');
      this.log('Console working temp directory created.');
      this.log('===============');
    });
  }
  ...
  cleanup() {
    ...
    this.log('Cleaning up temp dir...');
    return fs.pathExists(this.tmpDir)
    .then((exists) => {
      if (exists) {
        this.log('Removing ', this.tmpDir);
  ...
  createTempDir() {
    return new Promise((resolve, reject) => {
      tmp.dir((err, path) => {
        ...
        this.log('Created working directory', path);
        resolve(path);
      });
    });
  }
  ...
  copyLocalConsole() {
    ...
    .then((stats) => {
      ...
      return fs.copy(this.opts.src + '/*', this.tmpDir);
    });
  }
   ...
   */
  installBower() {
    this.log('Installing bower and dependencies...');
    ...
    .then(() => {
      if (!this.opts.useJson && this.opts.raml) {
        this.log('Installing parser and enhancer...');
        let cmd = 'bower install advanced-rest-client/raml-js-parser ';
        cmd += 'advanced-rest-client/raml-json-enhance';
  ...
  _setTemplateFile() {
    ...
    if (this.opts.mainFile) {
      this.log('Will not use a template.');
      return;
    }
    ...
    filename += '.tpl';
    this.log('Template to use: ', filename);
  ...
  handleTemplates() {
    ...
    this.log('Copy the template file to the build dir.');
    var src = path.join(__dirname, '..', 'templates', this.template);
    var dest = path.join(this.tmpDir, 'index.html');
    this.opts.mainFile = 'index.html';
    var rd = fs.createReadStream(src);
    var ws = fs.createWriteStream(dest);
    return new Promise((resolve, reject) => {
      let cleanUpReject = (err) => {
        rd.destroy();
        ws.end();
        reject(err);
      };
      rd.on('error', cleanUpReject);
      ws.on('error', cleanUpReject);
  ...
  updateTemplateVars(raml) {
    ...
    this.log('Updating template variables...');
    var file = path.join(this.tmpDir, 'index.html');
    ...
    .then(() => {
      this.log('Template variables updated.');
    });
  }
  /**
  ...
  _processTemplateVars(content, raml) {
    this.log('Replacing template variables with raml data...');
   ...
   */
  processMainFile() {
    this.log('Processing main file for an attributes...');
    var attributes = [];
    ...
    if (attributes.length === 0) {
      this.log('No attributes detected. Skipping.');
      return Promise.resolve();
    }
    ...
    .then((data) => {
      data = data.replace('<api-console>', `<api-console data-ac-build ${attributes}>`);
      return data;
    })
