...
class CacheBuild {
  ...
  constructor(opts, logger) {
    ...
    if (!this.opts.noCache) {
      ...
      this.logger.info('Build cache hash', this.hash);
  ...
  locateAppDir() {
    ...
    this.logger.info('Setting builds cache path to ', dir);
    return dir;
  }
  /**
  ...
  createHash(opts) {
    ...
    if (opts.api) {
      parts.push(`api=${opts.api}`);
    }
    if (opts.embedded) {
      parts.push(`embedded=${opts.embedded}`);
    }
    if (opts.withAmf) {
      parts.push(`withAmf=${opts.withAmf}`);
    }
    if (opts.noCryptoJs) {
   ...
   */
  cacheBuild(sources) {
    this.logger.info('Caching build files.');
  ...
  _createPackage(sources) {
    return new Promise((resolve, reject) => {
      ...
      output.on('close', () => {
        this.logger.info('Cache build saved.');
